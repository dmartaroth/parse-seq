...
# Subset chondro.1 and chondro.3 clusters
wnts <- c("Lrp5", "Axin2", "Nkd1", "Pax9", "Sfrp2")

obj$cell_types <- Idents(obj)

selected_cells_chondro1 <- names(obj$cell_types[obj$cell_types == "chondro.1"])
selected_cells_chondro3 <- names(obj$cell_types[obj$cell_types == "chondro.3"])

data_chondro1 <- FetchData(obj,
                           vars = c(wnts, "genotype"),
                           cells = selected_cells_chondro1,
                           layer = "data")
...
data_chondro1 <- data_chondro1[, c(wnts, "genotype")]
data_chondro3 <- data_chondro3[, c(wnts, "genotype")]

long_data_chondro1 <- reshape2::melt(data_chondro1)
long_data_chondro3 <- reshape2::melt(data_chondro3)

ggplot() +
  geom_violin(data = long_data_chondro1, aes(x = variable, y = value, fill = genotype, color = genotype, alpha = 0.4),
...
combined_data <- rbind(transform(long_data_chondro1, cluster = "chondro.1"),
                       transform(long_data_chondro3, cluster = "chondro.3"))
...
convenient_save_plot(combined_plot, "wnts_osteochondro-subset_vlnplot_by-genotype",width = 8, height = 4, file_format="pdf",dir = subset_osteochondro_dir)
